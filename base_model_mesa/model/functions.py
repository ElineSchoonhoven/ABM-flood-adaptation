...
def calculate_basic_flood_damage(flood_depth):
    ...
    """
    try:
        _ = len(flood_depth) #array operation test
        flood_damage = np.zeros_like(flood_depth)
        flood_damage[np.where(flood_depth > 6)] = 1
        flood_damage[np.where(flood_depth < 0)] = 0
        flood_damage[np.where((flood_depth > 0 ) & (flood_depth < 6))] = \
            0.1746 * np.log(flood_depth[np.where((flood_depth > 0 ) & (flood_depth < 6))]) + 0.6483
    except TypeError:
        if flood_depth >= 6:
            flood_damage = 1
            ...
            flood_damage = 0.1746 * np.log(flood_depth) + 0.6483
    return flood_damage

def calculate_economic_flood_damage(flood_depth, kws):
    return calculate_basic_flood_damage(flood_depth) * kws["housesize"] * kws["damage_m2"]
